						...
						for (k = 0; k < l1; k++) {
							...
							for (i = 2; i < ido; i += 2) {
								...
								ch[t3 - 1] = wa[index + idij - 1] * c1[t3 - 1]
										+ wa[index + idij] * c1[t3];
								ch[t3] = wa[index + idij - 1] * c1[t3] - wa[index + idij]
										* c1[t3 - 1];
							}
						}
					}
		...
		if (ido != 2) {
			...
			for (k = 0; k < l1; k++) {
				...
				for (i = 2; i < ido; i += 2) {
					...
					ch[(t8 += t0) - 1] = wa2[index2 + i - 2] * cr3 - wa2[index2 + i - 1]
							* ci3;
					ch[t8] = wa2[index2 + i - 2] * ci3 + wa2[index2 + i - 1] * cr3;
					ch[(t8 += t0) - 1] = wa3[index3 + i - 2] * cr4 - wa3[index3 + i - 1]
							* ci4;
			...
			case 132:
				...
				for (j = 1; j < ip; j++) {
					...
					for (i = 2; i < ido; i += 2) {
						...
						t3 = t2;
						for (k = 0; k < l1; k++) {
							c1[t3 - 1] = wa[index + idij - 1] * ch[t3 - 1] - wa[index + idij]
									* ch[t3];
							c1[t3] = wa[index + idij - 1] * ch[t3] + wa[index + idij]
									* ch[t3 - 1];
							t3 += ido;
						}
					}
		...
		for (k1 = 0; k1 < nf; k1++) {
			...
			loop: while (true) {
				...
				case 100:
					...
					if (na != 0)
						dradb4(ido, l1, ch, c, wa, index + iw - 1, wa, index + ix2 - 1, wa,
								index + ix3 - 1);
					else
						dradb4(ido, l1, c, ch, wa, index + iw - 1, wa, index + ix2 - 1, wa,
								index + ix3 - 1);
					na = 1 - na;
					state = 115;
					break;
