...
public class StreamState {
	...
	int pageno;
	long packetno; /*
									 * sequence number for decode; the framing knows where there's
									 * a hole in the data, but we need coupling so that the codec
									 * (which is in a seperate abstraction layer) also knows about
									 * the gap
									 */
	long granulepos;
	...
	public int packetin(Packet op) {
		...
		if (body_returned != 0) {
			/*
			 * advance packet data according to the body_returned pointer. We had to
			 * keep it around to return a pointer into the buffer last call
			 */
	...
	public int packetout(Packet op) {
		 ...
		 * Now we need to group them into packets (or return the out of sync
		 * markers)
		 */
	 ...
	 * there are no packets or partial packets to flush, ogg_stream_flush returns
	 * 0. Note that ogg_stream_flush will try to flush a normal sized page like
	 ...
	 * undersized; you almost certainly want to use ogg_stream_pageout (and *not*
	 ...
	 * until the next call (using the same ogg_stream_state)
	 */
