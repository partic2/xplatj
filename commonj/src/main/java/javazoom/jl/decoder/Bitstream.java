...
public final class Bitstream {
	...
	private void parseID3v2Frames(byte[] bframes) {
		...
		try {
			...
			for (int i = 10; i < bframes.length && bframes[i] > 0; i += size) {
				if (v2version == 3 || v2version == 4) {
					...
					size = (int) (bframes[i + 4] << 24 & 0xFF000000
							| bframes[i + 5] << 16 & 0x00FF0000 | bframes[i + 6] << 8
			...
			if (replayGain != null && replayGainPeak != null) {
				...
				// If scale * peak > 1 then reduce scale (preamp) to prevent
				// clipping.
		...
		try {
			...
			if (ex.getErrorCode() == INVALIDFRAME) {
				...
				try {
					...
					if (e.getErrorCode() != STREAM_EOF) // wrap original
						// exception so stack
						// trace is maintained.
	...
	public boolean isSyncCurrentPosition(int syncmode) throws BitstreamException {
		...
		int headerstring = syncbuf[0] << 24 & 0xFF000000 | syncbuf[1] << 16
				& 0x00FF0000 | syncbuf[2] << 8 & 0x0000FF00 | syncbuf[3] << 0
				& 0x000000FF;
