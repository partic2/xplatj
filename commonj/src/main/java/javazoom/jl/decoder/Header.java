	...
	void read_header(Bitstream stream, Crc16[] crcp) throws BitstreamException {
		...
		do {
			...
			if (h_layer == 1)
				...
				else if (h_sample_frequency == FOURTYEIGHT || channel_bitrate >= 3
						&& channel_bitrate <= 5)
			...
			if (framesize >= 0 && framesizeloaded != framesize) // Data loaded
				// does not
				// match to
				// expected
				// framesize,
	...
	private static final String bitrate_str[][][] = {
...

			{
					{ "free format", "32 kbit/s", "64 kbit/s", "96 kbit/s", "128 kbit/s",
							...
							"288 kbit/s", "320 kbit/s", "352 kbit/s", "384 kbit/s",
