...
public final class SynthesisFilter {
	...
	private void compute_pcm_samples0(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[0 + dvp] * dp[0] + vp[15 + dvp] * dp[1] + vp[14 + dvp]
					* dp[2] + vp[13 + dvp] * dp[3] + vp[12 + dvp] * dp[4] + vp[11 + dvp]
					* dp[5] + vp[10 + dvp] * dp[6] + vp[9 + dvp] * dp[7] + vp[8 + dvp]
					* dp[8] + vp[7 + dvp] * dp[9] + vp[6 + dvp] * dp[10] + vp[5 + dvp]
					* dp[11] + vp[4 + dvp] * dp[12] + vp[3 + dvp] * dp[13] + vp[2 + dvp]
	...
	private void compute_pcm_samples1(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[1 + dvp] * dp[0] + vp[0 + dvp] * dp[1] + vp[15 + dvp]
					* dp[2] + vp[14 + dvp] * dp[3] + vp[13 + dvp] * dp[4] + vp[12 + dvp]
					* dp[5] + vp[11 + dvp] * dp[6] + vp[10 + dvp] * dp[7] + vp[9 + dvp]
					* dp[8] + vp[8 + dvp] * dp[9] + vp[7 + dvp] * dp[10] + vp[6 + dvp]
					* dp[11] + vp[5 + dvp] * dp[12] + vp[4 + dvp] * dp[13] + vp[3 + dvp]
	...
	private void compute_pcm_samples2(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[2 + dvp] * dp[0] + vp[1 + dvp] * dp[1] + vp[0 + dvp]
					* dp[2] + vp[15 + dvp] * dp[3] + vp[14 + dvp] * dp[4] + vp[13 + dvp]
					* dp[5] + vp[12 + dvp] * dp[6] + vp[11 + dvp] * dp[7] + vp[10 + dvp]
					* dp[8] + vp[9 + dvp] * dp[9] + vp[8 + dvp] * dp[10] + vp[7 + dvp]
					* dp[11] + vp[6 + dvp] * dp[12] + vp[5 + dvp] * dp[13] + vp[4 + dvp]
	...
	private void compute_pcm_samples3(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[3 + dvp] * dp[0] + vp[2 + dvp] * dp[1] + vp[1 + dvp]
					* dp[2] + vp[0 + dvp] * dp[3] + vp[15 + dvp] * dp[4] + vp[14 + dvp]
					* dp[5] + vp[13 + dvp] * dp[6] + vp[12 + dvp] * dp[7] + vp[11 + dvp]
					* dp[8] + vp[10 + dvp] * dp[9] + vp[9 + dvp] * dp[10] + vp[8 + dvp]
					* dp[11] + vp[7 + dvp] * dp[12] + vp[6 + dvp] * dp[13] + vp[5 + dvp]
	...
	private void compute_pcm_samples4(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[4 + dvp] * dp[0] + vp[3 + dvp] * dp[1] + vp[2 + dvp]
					* dp[2] + vp[1 + dvp] * dp[3] + vp[0 + dvp] * dp[4] + vp[15 + dvp]
					* dp[5] + vp[14 + dvp] * dp[6] + vp[13 + dvp] * dp[7] + vp[12 + dvp]
					* dp[8] + vp[11 + dvp] * dp[9] + vp[10 + dvp] * dp[10] + vp[9 + dvp]
					* dp[11] + vp[8 + dvp] * dp[12] + vp[7 + dvp] * dp[13] + vp[6 + dvp]
	...
	private void compute_pcm_samples5(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[5 + dvp] * dp[0] + vp[4 + dvp] * dp[1] + vp[3 + dvp]
					* dp[2] + vp[2 + dvp] * dp[3] + vp[1 + dvp] * dp[4] + vp[0 + dvp]
					* dp[5] + vp[15 + dvp] * dp[6] + vp[14 + dvp] * dp[7] + vp[13 + dvp]
					* dp[8] + vp[12 + dvp] * dp[9] + vp[11 + dvp] * dp[10] + vp[10 + dvp]
					* dp[11] + vp[9 + dvp] * dp[12] + vp[8 + dvp] * dp[13] + vp[7 + dvp]
	...
	private void compute_pcm_samples6(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[6 + dvp] * dp[0] + vp[5 + dvp] * dp[1] + vp[4 + dvp]
					* dp[2] + vp[3 + dvp] * dp[3] + vp[2 + dvp] * dp[4] + vp[1 + dvp]
					* dp[5] + vp[0 + dvp] * dp[6] + vp[15 + dvp] * dp[7] + vp[14 + dvp]
					* dp[8] + vp[13 + dvp] * dp[9] + vp[12 + dvp] * dp[10] + vp[11 + dvp]
					* dp[11] + vp[10 + dvp] * dp[12] + vp[9 + dvp] * dp[13] + vp[8 + dvp]
	...
	private void compute_pcm_samples7(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[7 + dvp] * dp[0] + vp[6 + dvp] * dp[1] + vp[5 + dvp]
					* dp[2] + vp[4 + dvp] * dp[3] + vp[3 + dvp] * dp[4] + vp[2 + dvp]
					* dp[5] + vp[1 + dvp] * dp[6] + vp[0 + dvp] * dp[7] + vp[15 + dvp]
					* dp[8] + vp[14 + dvp] * dp[9] + vp[13 + dvp] * dp[10] + vp[12 + dvp]
					* dp[11] + vp[11 + dvp] * dp[12] + vp[10 + dvp] * dp[13]
	...
	private void compute_pcm_samples8(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[8 + dvp] * dp[0] + vp[7 + dvp] * dp[1] + vp[6 + dvp]
					* dp[2] + vp[5 + dvp] * dp[3] + vp[4 + dvp] * dp[4] + vp[3 + dvp]
					* dp[5] + vp[2 + dvp] * dp[6] + vp[1 + dvp] * dp[7] + vp[0 + dvp]
					* dp[8] + vp[15 + dvp] * dp[9] + vp[14 + dvp] * dp[10] + vp[13 + dvp]
					* dp[11] + vp[12 + dvp] * dp[12] + vp[11 + dvp] * dp[13]
	...
	private void compute_pcm_samples9(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[9 + dvp] * dp[0] + vp[8 + dvp] * dp[1] + vp[7 + dvp]
					* dp[2] + vp[6 + dvp] * dp[3] + vp[5 + dvp] * dp[4] + vp[4 + dvp]
					* dp[5] + vp[3 + dvp] * dp[6] + vp[2 + dvp] * dp[7] + vp[1 + dvp]
					* dp[8] + vp[0 + dvp] * dp[9] + vp[15 + dvp] * dp[10] + vp[14 + dvp]
					* dp[11] + vp[13 + dvp] * dp[12] + vp[12 + dvp] * dp[13]
	...
	private void compute_pcm_samples10(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[10 + dvp] * dp[0] + vp[9 + dvp] * dp[1] + vp[8 + dvp]
					* dp[2] + vp[7 + dvp] * dp[3] + vp[6 + dvp] * dp[4] + vp[5 + dvp]
					* dp[5] + vp[4 + dvp] * dp[6] + vp[3 + dvp] * dp[7] + vp[2 + dvp]
					* dp[8] + vp[1 + dvp] * dp[9] + vp[0 + dvp] * dp[10] + vp[15 + dvp]
					* dp[11] + vp[14 + dvp] * dp[12] + vp[13 + dvp] * dp[13]
	...
	private void compute_pcm_samples11(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[11 + dvp] * dp[0] + vp[10 + dvp] * dp[1] + vp[9 + dvp]
					* dp[2] + vp[8 + dvp] * dp[3] + vp[7 + dvp] * dp[4] + vp[6 + dvp]
					* dp[5] + vp[5 + dvp] * dp[6] + vp[4 + dvp] * dp[7] + vp[3 + dvp]
					* dp[8] + vp[2 + dvp] * dp[9] + vp[1 + dvp] * dp[10] + vp[0 + dvp]
					* dp[11] + vp[15 + dvp] * dp[12] + vp[14 + dvp] * dp[13]
	...
	private void compute_pcm_samples12(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[12 + dvp] * dp[0] + vp[11 + dvp] * dp[1] + vp[10 + dvp]
					* dp[2] + vp[9 + dvp] * dp[3] + vp[8 + dvp] * dp[4] + vp[7 + dvp]
					* dp[5] + vp[6 + dvp] * dp[6] + vp[5 + dvp] * dp[7] + vp[4 + dvp]
					* dp[8] + vp[3 + dvp] * dp[9] + vp[2 + dvp] * dp[10] + vp[1 + dvp]
					* dp[11] + vp[0 + dvp] * dp[12] + vp[15 + dvp] * dp[13]
	...
	private void compute_pcm_samples13(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[13 + dvp] * dp[0] + vp[12 + dvp] * dp[1] + vp[11 + dvp]
					* dp[2] + vp[10 + dvp] * dp[3] + vp[9 + dvp] * dp[4] + vp[8 + dvp]
					* dp[5] + vp[7 + dvp] * dp[6] + vp[6 + dvp] * dp[7] + vp[5 + dvp]
					* dp[8] + vp[4 + dvp] * dp[9] + vp[3 + dvp] * dp[10] + vp[2 + dvp]
					* dp[11] + vp[1 + dvp] * dp[12] + vp[0 + dvp] * dp[13] + vp[15 + dvp]
					* dp[14] + vp[14 + dvp] * dp[15]) * scalefactor);
	...
	private void compute_pcm_samples14(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			...
			pcm_sample = ((vp[14 + dvp] * dp[0] + vp[13 + dvp] * dp[1] + vp[12 + dvp]
					* dp[2] + vp[11 + dvp] * dp[3] + vp[10 + dvp] * dp[4] + vp[9 + dvp]
					* dp[5] + vp[8 + dvp] * dp[6] + vp[7 + dvp] * dp[7] + vp[6 + dvp]
					* dp[8] + vp[5 + dvp] * dp[9] + vp[4 + dvp] * dp[10] + vp[3 + dvp]
					* dp[11] + vp[2 + dvp] * dp[12] + vp[1 + dvp] * dp[13] + vp[0 + dvp]
					* dp[14] + vp[15 + dvp] * dp[15]) * scalefactor);
	...
	private void compute_pcm_samples15(OutputBuffer buffer) {
		...
		for (int i = 0; i < 32; i++) {
			float pcm_sample;
			final float dp[] = d16[i];
			pcm_sample = ((vp[15 + dvp] * dp[0] + vp[14 + dvp] * dp[1] + vp[13 + dvp]
					* dp[2] + vp[12 + dvp] * dp[3] + vp[11 + dvp] * dp[4] + vp[10 + dvp]
					* dp[5] + vp[9 + dvp] * dp[6] + vp[8 + dvp] * dp[7] + vp[7 + dvp]
					* dp[8] + vp[6 + dvp] * dp[9] + vp[5 + dvp] * dp[10] + vp[4 + dvp]
					* dp[11] + vp[3 + dvp] * dp[12] + vp[2 + dvp] * dp[13] + vp[1 + dvp]
	...
	static private float[] load_d() {
		try {
			...
			Object o = deserializeArray(
					SynthesisFilter.class.getResourceAsStream("/sfd.ser"), elemType, 512);
			return (float[]) o;
	 ...
	 */
	static private Object deserializeArray(InputStream in, Class<?> elemType,
